                 ...
                 <Container> {/* Title */}
                     {!Menu && <>
                        ...
                        <Box style={{width:"91%"}} boxShadow={2} className=" rounded-full  HaideResercheBar	mt-12 flex opacity-90 ml-9 bg-white">
                            ...
                            <img className="w-15 h-10 ml-1 mr-10 mt-1"  src={process.env.PUBLIC_URL +`/TESt05zd.png` }/>
                        ...
                        <Box className="px-10 py-3">
                            {Produtct.filter((el)=>el.Name.toLocaleLowerCase().includes(Catgories.toLocaleLowerCase().trim())).map((el)=><Link className="no-underline" to={`/ProductFilter/${el.Name}`} ><p>{el.Name}</p></Link>)}
                ...
                <Box data-aos="fade-down-left" className="ml-3p">
                   ...
                   {/* Here */}
                    ...
                    {Catgories && 
                        <Box  boxShadow={5}   style={{height:"150px"}} className="bg-gray-200 flex-left	 rounded-3xl flex	 AnimationMenu	bg-gray-200	 wp-30 mt-2  mb-10">
                        <Box className="px-10 py-3">
                            {Produtct.filter((el)=>el.Name.toLocaleLowerCase().includes(Catgories.toLocaleLowerCase().trim())).map((el)=> <Link className="no-underline" to={`/ProductFilter/${el.Name}`}><p className="f-14">{el.Name}</p> </Link> )}
